...
Chess-based SLM trained exclusively on position (FEN) + best move (UCI) pairs without move history context. A baseline for comparison against any claims of LLM "mastery" or "understanding" of chess.

Forked from [karpathy/nanoGPT](https://github.com/karpathy/nanogpt)

## Models

<!-- [![HuggingFace](https://img.shields.io/badge/🤗_HuggingFace-Datasets-yellow?style=for-the-badge)](https://huggingface.co/ethanjtang) -->

### Config

TODO
puzzles only config
sf selfplay only config
combined config
...
KINGPT variant trained on combined dataset of Woodpecker and Beaver variant positions.
Achieved TODO on combined corpus after training for ~500B tokens.

## Installation/Dependencies

This is taken directly from the nanoGPT repository. I would highly recommend just to follow the instructions there on how to hack it to your needs instead of reading my config.
...
<!-- [![HuggingFace](https://img.shields.io/badge/🤗_HuggingFace-Puzzles-yellow?style=for-the-badge)](https://huggingface.co/ethanjtang) -->

<!-- [![HuggingFace](https://img.shields.io/badge/🤗_HuggingFace-SF18%20Selfplay%20Games-yellow?style=for-the-badge)](https://huggingface.co/ethanjtang) -->
...
- (Mandatory) Play Atlus's Metaphor: ReFantazio (~90 hours)
- (Optional) Question if training a chess-focused large language model is even worth it when we have Stockfish (+ the many other engines) as external verifiers, which can either be called directly by general LLMs (via a wrapper) or utilized in an LLM-Modulo system (as an external verifier to check LLM generation).
